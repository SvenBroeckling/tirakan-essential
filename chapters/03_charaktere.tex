...
\begin{multicols*}{2}
\section{Was einen Charakter ausmacht}
...
\begin{itemize}[leftmargin=*]
    ...
    \item drei Start-Prägungen: \textbf{Abstammung}, \textbf{Weg}, \textbf{Bindung}
    ...
    \item den Leisten \textbf{Wunden}, \textbf{Bürde}, \textbf{Omen} sowie gegebenenfalls \textbf{Arkana}, \textbf{Gunst} und \textbf{Verderbnis}
\end{itemize}
...
\subsection{Schritt 3: Attribute}
...
\subsection{Schritt 4: Fertigkeiten}
...
\subsection{Schritt 5: Prägungen}
Wähle:
\begin{itemize}[leftmargin=*]
...
Mögliche Prägungen stehen gesammelt in \cref{app:marks}.

\subsection{Schritt 6: Schuld oder Eid}
...
\subsection{Schritt 7: Abgeleitete Werte}

Berechne die abgeleiteten Werte für den Charakter, und trage sie auf dem Charakterbogen ein.

\begin{description}[style=nextline,leftmargin=0pt,itemsep=0.2em]
    \item[Wundenschwelle] $3 + \text{Körper}$
    \item[Bürdenschwelle] $5 + \lfloor \text{Wille} / 2 \rfloor$
    \item[Initiative Zielwurf] $30 + (\text{Geschick} \times 10) + (\text{Wahrnehmung} \times 10)$
    \item[Omen] $2 + \lfloor \text{Glaubensniveau} / 2 \rfloor$
    \item[Zauberverstärkung] $\text{Magieniveau}$
    \item[Anrufungswert] $\text{Glaubensniveau} + \text{Ritus}$
    \item[Gunstgrenze je Anrufung] $1 + \lfloor \text{Wille} / 2 \rfloor$
\end{description}

\subsection{Schritt 8: Startausrüstung}
Wähle eines der fünf Startpakete:
\begin{description}[style=nextline,leftmargin=0pt]
\item[\textbf{Jäger}] Speer, Lederwams, Fallenwerkzeug, Seil (10m), Ölflasche
\item[\textbf{Büßer}] Streitkolben, Flickenpanzer, Weihrauchbündel, Gebetsschnur, Salbendose
\item[\textbf{Straßenwächter}] Schwert, Brigantine, Laterne, Kreide, Kette (5m)
\item[\textbf{Hofagent}] Dolch, Pistole, Gepolsterter Mantel, Siegelring, Chiffrierbuch, Giftfläschchen
\item[\textbf{Grabesgelehrter}] Stab, Geschichtete Roben, Reliktenwerkzeug, Kreidesalze, Knochenkarten
\end{description}

Alternativ darf mit Zustimmung der Spielleiterin ein gleichwertiges Paket gebaut werden:
\begin{itemize}[leftmargin=*]
...
\subsection{Schritt 9: Startwerte}
...
\subsection{Schritt 10: Übernatürlicher Zugang}
...
\textbf{Zauberei:}
\begin{itemize}[leftmargin=*]
\item bei Zauberei >= 1: eine Schule wahlen
\item bei Zauberei >= 2: zwei Schulen wahlen
